...
public class Grid : MonoBehaviour {
	...
	void CheckForDisconnected () {
		//nastavi vsechny bubliny jako nepripojene
		foreach (var r in gridBalls) {
			foreach (var b in r) {
				b.connected = false;
			}
		}
		//spoji bubliny v horni rade
		foreach (var b in gridBalls[0]) {
			if (b.gameObject.activeSelf)
				b.connected = true;
		}

		//nastavi vsechny ostatni rady
		var i = 1;
		while (i < ROWS) {
			foreach (var b in gridBalls[i]) {
				if (b.gameObject.activeSelf) {
